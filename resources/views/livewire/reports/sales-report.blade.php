    ...
    <div class="p-4 bg-white dark:bg-gray-800">
        <div class="mb-4">
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                ...
                @if(count($availableRestaurants ?? []) > 1)
                    <span class="ml-1 font-semibold">
                        -
                        @if($selectedRestaurantId)
                            {{ optional(collect($availableRestaurants)->firstWhere('id', (int)$selectedRestaurantId))->name }}
                        @else
                            Toutes les entreprises liees
                        @endif
                    </span>
                @endif
            </p>
        ...
        <div class="flex flex-wrap justify-between items-center gap-4 p-4 bg-gray-50 rounded-lg dark:bg-gray-700">
            ...
            <div class="inline-flex items-center gap-2 w-1/2 sm:w-auto ms-2">
                ...
                @if(count($availableRestaurants ?? []) > 1)
                    <select wire:model.live="selectedRestaurantId" class="px-3 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-4 focus:ring-primary-300 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-700">
                        <option value="">Toutes les entreprises liees</option>
                        @foreach($availableRestaurants as $linkedRestaurant)
                            <option value="{{ $linkedRestaurant->id }}">{{ $linkedRestaurant->name }}</option>
                        @endforeach
                    </select>
                @endif
