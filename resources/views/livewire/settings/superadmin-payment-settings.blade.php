    ...
    <div
        ...
        <div class="text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700">
            <ul class="flex flex-wrap items-center -mb-px">
                ...
                <li wire:click="activeSetting('fedapay')" class="me-2">
                    <span @class([
                        ...
                        'border-transparent' => ($activePaymentSetting != 'fedapay'),
                        'active border-skin-base dark:text-skin-base dark:border-skin-base text-skin-base' => ($activePaymentSetting == 'fedapay')
                    ])>
                        <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 40" fill="none">
                            <rect width="120" height="40" rx="6" fill="#0f766e"/>
                            <text x="60" y="25" font-size="16" text-anchor="middle" fill="#fff">FedaPay</text>
                        </svg>
                        FedaPay
                        <span @class([
                            'flex w-3 h-3 me-3 rounded-full',
                            'bg-green-500' => $fedapayStatus,
                            'bg-red-500' => ! $fedapayStatus
                        ])></span>
                    </span>
                </li>
       ...
       @if($activePaymentSetting == 'fedapay')
            <form wire:submit="submitFormFedapay">
                <div class="grid gap-6">

                    <div class="my-3">
                        <x-label for="fedapayStatus">
                            <div class="flex items-center cursor-pointer">
                                <x-checkbox name="fedapayStatus" id="fedapayStatus" wire:model.live="fedapayStatus"/>
                                <div class="ms-2">
                                    Enable FedaPay
                                </div>
                            </div>
                    ...
                    @if ($fedapayStatus)
                        <div>
                            <x-label for="fedapayMode" :value="__('modules.settings.selectEnvironment')"/>
                            <x-select id="fedapayMode" class="block w-full mt-1" wire:model.live="fedapayMode">
                                <option value="sandbox">@lang('app.test')</option>
                                <option value="live">@lang('app.live')</option>
                            </x-select>
                            <x-input-error for="fedapayMode" class="mt-2"/>
                        </div>

                        @if ($fedapayMode == 'live')
                            <div class="grid grid-cols-2 gap-x-4">
                                <div>
                                    <x-label for="fedapayPublicKey" value="FedaPay Public Key" required/>
                                    <x-input id="fedapayPublicKey" class="block w-full mt-1" type="text" wire:model="fedapayPublicKey"/>
                                    <x-input-error for="fedapayPublicKey" class="mt-2"/>
                                </div>

                                <div>
                                    <x-label for="fedapaySecretKey" value="FedaPay Secret Key" required/>
                                    <x-input-password id="fedapaySecretKey" class="block w-full mt-1" type="text" wire:model="fedapaySecretKey"/>
                                    <x-input-error for="fedapaySecretKey" class="mt-2"/>
                                </div>
                            </div>
                        @else
                            <div class="grid grid-cols-2 gap-x-4">
                                <div>
                                    <x-label for="testFedapayPublicKey" value="Test FedaPay Public Key" required/>
                                    <x-input id="testFedapayPublicKey" class="block w-full mt-1" type="text" wire:model="testFedapayPublicKey"/>
                                    <x-input-error for="testFedapayPublicKey" class="mt-2"/>
                                </div>

                                <div>
                                    <x-label for="testFedapaySecretKey" value="Test FedaPay Secret Key" required/>
                                    <x-input-password id="testFedapaySecretKey" class="block w-full mt-1" type="text" wire:model="testFedapaySecretKey"/>
                                    <x-input-error for="testFedapaySecretKey" class="mt-2"/>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="mt-4">
                            <x-label :value="__('modules.settings.webhookUrl')" class="mb-1"/>
                            <div class="flex items-center">
                                <span class="relative px-1 py-1 font-medium transition duration-300 bg-gray-100 rounded cursor-pointer purchase-code dark:text-white group" id="webhook-url-fedapay">
                                    {{$webhookUrl}}
                                </span>
                                <button id="copy-button" type="button" onclick="copyWebhookUrl('webhook-url-fedapay')" class="px-3 py-2 ml-2 text-white bg-gray-800 rounded-lg hover:bg-gray-700">
                                    @lang('modules.settings.copyWebhookUrl')
                                </button>
                            </div>
                        </div>
