    ...
    @if ($showMenu && !$showOrderTypeModal)
        <div class="px-4 mb-32 space-y-4 lg:gap-8"
            ...
            @scroll.window.throttle.200ms="scrollHandler()">
                ...
                @forelse (
                    $this->menuItems->map(fn ($items) => $items->reject(fn ($item) => str_contains(Str::lower(trim($item->item_name ?? '')), 'timbre fiscal'))->values())
                 ...
                 @else
                    <div class="grid items-center w-full grid-cols-1 gap-6 mt-4 md:grid-cols-2">
                        ...
                        @if ($paymentGateway->fedapay_status)
                            <x-secondary-button wire:click='initiateFedapayPayment({{ $paymentOrder->id }})'>
                                <span class="inline-flex items-center">
                                    <svg class="w-4 h-4 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <rect x="3" y="3" width="18" height="18" rx="4" stroke="currentColor" stroke-width="2"/>
                                        <path d="M8 12h8M8 8h5M8 16h3" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                                    </svg>
                                    FedaPay
                                </span>
                            </x-secondary-button>
