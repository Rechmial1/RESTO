<div>
    <x-dialog-modal wire:model.live="showAddPaymentModal" maxWidth="4xl" class="w-full max-w-full">
        <x-slot name="title">
            @if($order)
            @php
                $isCreditNote = $order && $order->invoice_type === 'FA';
            @endphp
            ...
            <div x-data="{
                ...
                <div x-show="!showSplitOptions" class="flex flex-col md:flex-row gap-4 flex-wrap w-full">
                    ...
                    <div class="w-full md:flex-1">
                        ...
                        <button
                            class="flex-1 py-2 px-3 text-center border rounded-lg hover:bg-gray-50 text-sm {{ $isCreditNote ? 'opacity-60 cursor-not-allowed' : '' }}"
                            @if(!$isCreditNote)
                                wire:click="showSplitOptions(true)"
                                @click="showSplitOptions = true"
                            @endif
                            ...
                            <!-- Payment Methods - ESPÈCES en premier et par défaut -->
                            <div @class([
                                'grid gap-3',
                                'grid-cols-2 sm:grid-cols-4' => !$canAddTip,
                                'grid-cols-2 sm:grid-cols-5' => $canAddTip
                            ])>
                                <!-- Bouton Espèces (existant) -->
                                <button
                                    @if(!$isCreditNote) wire:click="setPaymentMethod('cash')" @endif
                                    @disabled($isCreditNote)
                                    class="p-3 text-center border rounded-lg {{ $paymentMethod === 'cash' ? 'bg-skin-base/5 border-skin-base ring-2 ring-skin-base/20' : 'hover:bg-gray-50' }} {{ $isCreditNote ? 'opacity-60 cursor-not-allowed' : '' }}">
                                    <svg class="w-6 h-6 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2m2 4h10a2 2 0 0 0 2-2v-6a2 2 0 0 0-2-2H9a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2m7-5a2 2 0 1 1-4 0 2 2 0 0 1 4 0"/>
                                    </svg>
                                    <span class="text-sm font-medium">@lang('modules.order.cash')</span>
                                    @if($paymentMethod === 'cash')
                                        <span class="block text-xs text-skin-base mt-1">✓ Par défaut</span>
                                    @endif
                                </button>

                                <!-- Bouton Carte (existant) -->
                                <button
                                    @if(!$isCreditNote) wire:click="setPaymentMethod('card')" @endif
                                    @disabled($isCreditNote)
                                    class="p-3 text-center border rounded-lg {{ $paymentMethod === 'card' ? 'bg-skin-base/5 border-skin-base' : 'hover:bg-gray-50' }} {{ $isCreditNote ? 'opacity-60 cursor-not-allowed' : '' }}">
                                    <svg class="w-6 h-6 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 0 0 3-3V8a3 3 0 0 0-3-3H6a3 3 0 0 0-3 3v8a3 3 0 0 0 3 3"/>
                                    </svg>
                                ...
                                <!-- Bouton UPI (existant) -->
                                <button
                                    @if(!$isCreditNote) wire:click="setPaymentMethod('upi')" @endif
                                    @disabled($isCreditNote)
                                    class="p-3 text-center border rounded-lg {{ $paymentMethod === 'upi' ? 'bg-skin-base/5 border-skin-base' : 'hover:bg-gray-50' }} {{ $isCreditNote ? 'opacity-60 cursor-not-allowed' : '' }}">
                                ...
                                <!-- Bouton Virement Bancaire (existant) -->
                                <button
                                    @if(!$isCreditNote) wire:click="setPaymentMethod('bank_transfer')" @endif
                                    @disabled($isCreditNote)
                                    class="p-3 text-center border rounded-lg {{ $paymentMethod === 'bank_transfer' ? 'bg-skin-base/5 border-skin-base' : 'hover:bg-gray-50' }} {{ $isCreditNote ? 'opacity-60 cursor-not-allowed' : '' }}">
                                    <svg class="w-6 h-6 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- ========== NOUVEAU BOUTON FEDAPAY ========== -->
                                <button
                                    @if(!$isCreditNote) wire:click="setPaymentMethod('fedapay')" @endif
                                    @disabled($isCreditNote)
                                    class="p-3 text-center border rounded-lg {{ $paymentMethod === 'fedapay' ? 'bg-skin-base/5 border-skin-base ring-2 ring-skin-base/20' : 'hover:bg-gray-50' }} {{ $isCreditNote ? 'opacity-60 cursor-not-allowed' : '' }}">
                                    <svg class="w-6 h-6 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span class="text-sm font-medium">FedaPay</span>
                                    @if($paymentMethod === 'fedapay')
                                        <span class="block text-xs text-skin-base mt-1">✓ Sélectionné</span>
                                    @endif
                                </button> 

                                <button
                                    @if(!$isCreditNote) wire:click="setPaymentMethod('mobilemoney')" @endif
                                    @disabled($isCreditNote)
                                    class="p-3 text-center border rounded-lg {{ $paymentMethod === 'mobilemoney' ? 'bg-skin-base/5 border-skin-base ring-2 ring-skin-base/20' : 'hover:bg-gray-50' }} {{ $isCreditNote ? 'opacity-60 cursor-not-allowed' : '' }}">
                                    <svg class="w-6 h-6 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/>
                                    </svg>
                                ...
                                <!-- Bouton Paiement dû (existant) -->
                                <button
                                    @if(!$isCreditNote) wire:click="setPaymentMethod('due')" @endif
                                    @disabled($isCreditNote)
                                    class="p-3 text-center border rounded-lg {{ $paymentMethod === 'due' ? 'bg-skin-base/5 border-skin-base' : 'hover:bg-gray-50' }} {{ $isCreditNote ? 'opacity-60 cursor-not-allowed' : '' }}">
                                    <svg class="w-6 h-6 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button wire:click="addTipModal"
                                    ...
                                    <svg class="w-6 h-6 mx-auto mb-1 {{ $order && $order->tip_amount > 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-400 dark:text-gray-500' }}"
                                        ...
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0"/>
                                    </svg>
                            ...
                            @if($isCreditNote)
                                <div class="mt-3 text-sm text-amber-600 dark:text-amber-400">
                                    Le mode de paiement initial d'une facture avoir ne peut pas etre modifie.
                                </div>
                            @endif
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    @if($isCreditNote) readonly @endif
                                    class="w-full text-2xl p-3 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200 {{ $isCreditNote ? 'bg-gray-100 cursor-not-allowed' : '' }}"
                                >
                            </div>

                            @if($isCreditNote)
                                <div class="mt-2 text-sm text-amber-600 dark:text-amber-400">
                                    Le montant d'un retour est fixe et ne peut pas etre modifie.
                                </div>
                            @endif
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 my-2 text-sm sm:text-base">
                                ...
                                <!-- TIMBRE FISCAL - Affichage si appliqué -->
                                @if($this->timbreFiscalDetails)
                                <div class="mt-2 p-2 bg-blue-50 border border-blue-200 rounded">
                                    <div class="flex justify-between text-sm">
                                        <span class="text-blue-700 flex items-center gap-1">
                                            ...
                                            Timbre Fiscal ({{ $timbreFiscalRate }}%)
                                        </span>
                                        <span class="font-medium text-blue-700">{{ $this->timbreFiscalDetails['formatted'] }}</span>
                                    </div>
                                </div>
                                @endif
