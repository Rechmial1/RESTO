<div>
            ...
            @if ($order)
                ...
                @else
                    ...
                    <div class="flex flex-col space-y-4">
                        ...
                        @if(user_can('Update Order') && $order->invoice_type != 'FA')
                ...
                <div>
                    ...
                    <div class="w-full h-auto pt-3 pb-4 select-none">
                        ...
                        <div class="grid grid-cols-4 gap-2">
                            @if($order && in_array($order->status, ['paid']) && in_array($order->sfe_status, ['0']))
                                <x-secondary-button
                                    wire:click="normaliseOrder({{ $order->id }})"
                                    class="min-h-[50px] cursor-pointer rounded-xl bg-white hover:bg-gray-50
                                            text-gray-700 border p-3 inline-flex flex-col items-center
                                    ...
                                    <svg x-show="!loading" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="currentColor"
                                        viewBox="0 0 24 24">
                                        <path d="M4 3h16a2 2 0 0 1 2 2v3H2V5a2 2 0 0 1 2-2zm18 7H2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-9zm-14 3h4v2H8v-2zm6 0h2v2h-2v-2z"/>
                                    </svg>
                                    
                                    <svg x-show="loading" class="w-6 h-6 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>

                                    <span class="text-sm font-medium" x-text="loading ? 'Normalisation...' : 'Normaliser'"></span>

                                </x-secondary-button>
                            @endif

                            @if($order && $order->status == 'paid' && $order->invoice_type != 'FA' && in_array($order->sfe_status, ['1']))
                                <x-secondary-button
                                    wire:click="createReturnOrder({{ $order->id }})"
                                    class="min-h-[50px] cursor-pointer rounded-xl bg-orange-50 hover:bg-orange-100
                                            text-orange-700 border border-orange-300 p-3 inline-flex flex-col items-center
                                            justify-center gap-1 transition-colors">

                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 00-2-2H6a2 2 0 00-2 2v16l4-2 4 2 4-2 4 2z" />
                                    </svg>

                                    <span class="text-sm font-medium">Faire un retour</span>
                            ...
                            @if (in_array($order->status, ['billed', 'payment_due', 'paid']) && in_array($order->sfe_status, ['0']) && user_can('Delete Order'))
                                <button
                                ...
                                @foreach ($order->payments as $key => $item)
                                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                        ...
                                        <td @class([
                                            ...
                                            @php
                                                $lockedPaymentMethods = ['fedapay'];
                                                $canEditPaymentMethod =
                                                    $order->status !== 'pending_verification' &&
                                                    user_can('Update Order') &&
                                                    $order->invoice_type != 'FA' &&
                                                    in_array($order->sfe_status, ['0']) &&
                                                    !in_array($item->payment_method, $lockedPaymentMethods);
                                            ...
                                            @endphp
                                                ...
                                                <div @class([
                                                    ...
                                                    'bg-emerald-100 text-emerald-800 dark:bg-emerald-700 dark:text-emerald-300 border border-emerald-400' => $item->payment_method == 'fedapay',
                                                    'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400 border border-gray-400' => !in_array($item->payment_method, ['due', 'fedapay']),
                                                ])>
    ...
    @if ($order)
        ...
        <x-slot name="footer">
             ...
             <!-- Modal de retour -->
<!-- Modal de retour -->
            <x-dialog-modal wire:model.live="showReturnModal" maxWidth="4xl">
                <x-slot name="title">
                    <div class="flex items-center gap-3">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-orange-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M16 15v-1a4 4 0 0 0-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v16l4-2 4 2 4-2 4 2z" />
                        </svg>
                        <h2 class="text-xl font-semibold">Créer un retour sur facture</h2>
                    </div>
                    @if($originalOrder)
                    <div class="mt-2 text-sm text-gray-600">
                        Facture originale: <span class="font-medium">{{ $originalOrder->formatted_order_number ?? '' }}</span>
                        du {{ $originalOrder ? $originalOrder->date_time->format('d/m/Y H:i') : '' }}
                    </div>
                    @endif
                </x-slot>

                <x-slot name="content">
                    @if($originalOrder && !empty($itemsToReturn))
                    <div class="space-y-4">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Total retour</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($itemsToReturn as $index => $item)
                                    <tr>
                                        <td class="px-4 py-2">
                                        ...
                                        <td class="px-4 py-2 text-center">
                                            <span class="text-sm text-gray-600">{{ $item['max_quantity'] }}</span>
                                        </td>
                                        ...
                                        <td class="px-4 py-2 text-right font-medium">
                                            {{ currency_format($item['price'] * $item['max_quantity'], restaurant()->currency_id) }}
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="mt-4">
                            <x-label for="returnReason" value="Raison du retour *" />
                            <x-textarea
                                id="returnReason"
                        ...
                        <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                            <div class="flex justify-between items-center">
                                <span class="font-medium">Total à rembourser:</span>
                                <span class="text-2xl font-bold text-orange-600">
                                    {{ currency_format($returnTotal, restaurant()->currency_id) }}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <x-slot name="footer">
                    <div class="flex justify-end gap-3">
                        <x-button-cancel wire:click="closeReturnModal">
                            Annuler
                        </x-button-cancel>
                        <x-button wire:click="confirmReturn"
                                class="bg-orange-600 hover:bg-orange-700"
                                wire:loading.attr="disabled">
                            <span wire:loading.remove>Confirmer le retour</span>
