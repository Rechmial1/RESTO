...
class Pos extends Component
{
    ...
    public $timbreFiscalItemId = null; // ID de l'article "Timbre Fiscal" dans la base
    ...
    public function mount()
    {
        ...
        $timbreFiscalItem = MenuItem::where('item_name', 'like', '%Timbre Fiscal%')->first();
        if ($timbreFiscalItem) {
    ...
    public function calculateTotal()
    {
        ...
        // Calculer d'abord le total sans le timbre fiscal
        if (is_array($this->orderItemAmount)) {
         ...
         // 🔴 AJOUT : Vérifier si le timbre fiscal doit être appliqué
        $totalBeforeTimbre = $this->total;
        
        if ($totalBeforeTimbre > 100000 && !$this->isTimbreFiscalApplied) {
            $this->applyTimbreFiscal($totalBeforeTimbre);
        } elseif ($totalBeforeTimbre <= 100000 && $this->isTimbreFiscalApplied) {
            $this->removeTimbreFiscal();
        }
    ...
    /**
 * Applique le timbre fiscal (1%) au panier
 */
private function applyTimbreFiscal($totalAmount)
{
    if (!$this->timbreFiscalItemId) {
        // Si l'article n'existe pas, le créer automatiquement
        $this->createTimbreFiscalItem();
    }

    $timbreAmount = round($totalAmount * ($this->timbreFiscalRate / 100), 2);
    
    // Créer une clé unique pour l'article timbre fiscal
    $timbreKey = 'timbre_fiscal_' . time();
    
    // Ajouter l'article timbre fiscal au panier
    $timbreItem = MenuItem::find($this->timbreFiscalItemId);
    if ($timbreItem) {
        $this->orderItemList[$timbreKey] = $timbreItem;
        $this->orderItemQty[$timbreKey] = 1;
        $this->orderItemAmount[$timbreKey] = $timbreAmount;
        $this->orderItemModifiersPrice[$timbreKey] = 0;
        
        $this->timbreFiscalAmount = $timbreAmount;
        $this->isTimbreFiscalApplied = true;
        
        Log::info("💰 Timbre fiscal appliqué: " . number_format($timbreAmount, 0, ',', ' ') . " FCFA");
    }
}

/**
 * Supprime le timbre fiscal du panier
 */
private function removeTimbreFiscal()
{
    foreach ($this->orderItemList as $key => $item) {
        if (strpos($key, 'timbre_fiscal_') === 0) {
    ...
    $this->isTimbreFiscalApplied = false;
    
    Log::info("💰 Timbre fiscal retiré");
}

/**
 * Crée automatiquement l'article "Timbre Fiscal" s'il n'existe pas
 */
private function createTimbreFiscalItem()
{
    $timbreItem = MenuItem::create([
        'item_name' => 'Timbre Fiscal (1%)',
        'price' => 0,
        'type' => 'other',
        'menu_id' => $this->menuList->first()->id ?? 1,
        'item_category_id' => ItemCategory::first()->id ?? 1,
        'is_available' => true,
        'tax_inclusive' => false,
        'description' => 'Timbre fiscal de 1% pour les montants supérieurs à 100 000 FCFA'
    ]);
...
/**
 * Vérifie et met à jour le timbre fiscal après chaque modification du panier
 */
    public function checkTimbreFiscal()
    {
        $totalWithoutTimbre = 0;
        
        foreach ($this->orderItemAmount as $key => $amount) {
            if (strpos($key, 'timbre_fiscal_') !== 0) {
                $totalWithoutTimbre += $amount;
            }
        }
        
        if ($totalWithoutTimbre > 100000 && !$this->isTimbreFiscalApplied) {
            $this->applyTimbreFiscal($totalWithoutTimbre);
        } elseif ($totalWithoutTimbre <= 100000 && $this->isTimbreFiscalApplied) {
            $this->removeTimbreFiscal();
        }
    }
