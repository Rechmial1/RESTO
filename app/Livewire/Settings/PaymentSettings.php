...
class PaymentSettings extends Component
{
    ...
    public $fedapayStatus;
    public $fedapayMode;
    public $fedapayPublicKey;
    public $fedapaySecretKey;
    public $testFedapayPublicKey;
    public $testFedapaySecretKey;
    public $isFedapayEnabled;
    public bool $isGlobalFedapayEnabled = false;
    ...
    public function mount()
    {
        ...
        $this->isGlobalFedapayEnabled = (bool) $settings->enable_fedapay;
    ...
    private function setDefaultActivePaymentSetting()
    {
        ...
        $paymentGateways = [
            ...
            'fedapay' => $this->isGlobalFedapayEnabled,
            'offline' => true,
            'qr_code' => true,
    ...
    private function setCredentials()
    {
        ...
        $this->fedapayStatus = (bool) ($this->paymentGateway->fedapay_status ?? false);
        $this->fedapayMode = $this->paymentGateway->fedapay_mode ?? 'sandbox';
        $this->fedapayPublicKey = $this->paymentGateway->fedapay_public_key ?? null;
        $this->fedapaySecretKey = $this->paymentGateway->fedapay_secret_key ?? null;
        $this->testFedapayPublicKey = $this->paymentGateway->test_fedapay_public_key ?? null;
        $this->testFedapaySecretKey = $this->paymentGateway->test_fedapay_secret_key ?? null;
        $this->isFedapayEnabled = $this->paymentGateway->fedapay_status ?? false;
        ...
        if ($this->activePaymentSetting === 'fedapay') {
            $this->webhookUrl = route('fedapay.webhook', ['hash' => $hash]);
        }
...
public function submitFormFedapay()
{
    $this->validate([
        'fedapayStatus' => 'required|boolean',
        'fedapayMode' => 'required_if:fedapayStatus,true',
        'fedapayPublicKey' => 'required_if:fedapayStatus,true|required_if:fedapayMode,live',
        'fedapaySecretKey' => 'required_if:fedapayStatus,true|required_if:fedapayMode,live',
        'testFedapayPublicKey' => 'required_if:fedapayStatus,true|required_if:fedapayMode,sandbox',
        'testFedapaySecretKey' => 'required_if:fedapayStatus,true|required_if:fedapayMode,sandbox',
    ]);

    if ($this->saveFedapaySettings() === 0) {
        $this->updatePaymentStatus();
        $this->alertSuccess();
    }
}

private function saveFedapaySettings()
{
    if (! $this->fedapayStatus) {
        $this->paymentGateway->update([
            'fedapay_status' => $this->fedapayStatus,
        ]);
    ...
    try {
        $this->paymentGateway->update([
            'fedapay_status' => $this->fedapayStatus,
            'fedapay_mode' => $this->fedapayMode,
            'fedapay_public_key' => $this->fedapayPublicKey,
            'fedapay_secret_key' => $this->fedapaySecretKey,
            'test_fedapay_public_key' => $this->testFedapayPublicKey,
            'test_fedapay_secret_key' => $this->testFedapaySecretKey,
        ]);

        return 0;
    } catch (\Exception $e) {
        $field = $this->fedapayMode === 'live' ? 'fedapayPublicKey' : 'testFedapayPublicKey';
