...
class Restaurant extends BaseModel
{
    ...
    protected static function booted()
    {
        static::creating(function ($restaurant) {
            if (empty($restaurant->association_code)) {
                $restaurant->association_code = self::generateAssociationCode();
            }
        });
    }

    public static function generateAssociationCode(): string
    {
        do {
            $code = Str::upper(Str::random(5));
        } while (self::withoutGlobalScopes()->where('association_code', $code)->exists());
    ...
    public function primaryRestaurant(): BelongsTo
    {
        return $this->belongsTo(Restaurant::class, 'primary_restaurant_id')->withoutGlobalScopes();
    }

    public function secondaryRestaurants(): HasMany
    {
        return $this->hasMany(Restaurant::class, 'primary_restaurant_id')->withoutGlobalScopes();
    }
